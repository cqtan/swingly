...
function formatTime(timeString) {
  ...
  const startDate = new Date(
    `${startMonth} ${startDay}, ${year} ${startTime}:00`
  )
...
const getDetails = async detailItems => {
  ...
  for (let detailItem of detailItems) {
    ...
    if ((await detailItem.$("div > span.event-where")) !== null)
      location = await detailItem.$eval(
        "div > span.event-where",
        el => el.innerText
      )

    if ((await detailItem.$("div > span.event-where > a")) !== null)
      mapLink = await detailItem.$eval("div > span.event-where > a", el =>
        el.getAttribute("href")
      )

    if ((await detailItem.$("div > span.event-description")) !== null) {
      description = await detailItem.$eval(
        "div > span.event-description",
        el => el.innerText
      )
      ...
      for (let eventRow of eventRows) {
        if ((await eventRow.$("td.st-c")) !== null) {
          ...
          for (let eventWrapper of eventsWrapper) {
            ...
            const detailItems = await nestedFrame.$$(
              "div.details > div.detail-content > div"
            )
            ...
            if (
              (await nestedFrame.$(
                'div[id="bubbleContent:1"] > div.details > span.title'
              )) !== null
            )
              title = await nestedFrame.$eval(
                'div[id="bubbleContent:1"] > div.details > span.title',
                el => el.innerText
              )
...
const formatEvents = events => {
  ...
  let newEvents = events.map((event, idx) => {
    ...
    return {
      ...
      images: [],
      cancelled: false,
      hosts: ["wrlw2iC2tsOrM02iLm6OyUXfIv33"],
      type: "social",
      currency: "euro",
      courses: [],
      otherFees: [],
